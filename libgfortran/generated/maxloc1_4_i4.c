	...
	const index_type * const restrict pdim)
{
  ...
  if (retarray->base_addr == NULL)
	...
	runtime_error ("rank of return array incorrect in MAXLOC intrinsic: is %ld, should be %ld",
		       (long int) (GFC_DESCRIPTOR_RANK (retarray)),
		       (long int) rank);

      if (unlikely (compile_options.bounds_check))
	bounds_ifunction_return ((array_t *) retarray, extent, _("return value"), "MAXLOC");
    }
	...
	GFC_LOGICAL_4 * mask)
{
  ...
  if (retarray->base_addr == NULL)
    ...
    {
      if (rank != GFC_DESCRIPTOR_RANK (retarray))
	runtime_error (_("rank of return array incorrect in MAXLOC intrinsic: is %ld, should be %ld"),
		       (long int) (GFC_DESCRIPTOR_RANK (retarray)),
		       (long int) rank);
      ...
      if (unlikely (compile_options.bounds_check))
	{
	  for (n=0; n < rank; n++)
		...
		runtime_error (_("Incorrect extent in return value of MAXLOC intrinsic in dimension %ld: is %ld, should be %ld"),
