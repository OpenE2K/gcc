    ...
    read_sf (st_parameter_dt *dtp, int *length)
{
    ...
    while (n < *length)
    {
        ...
        else if (q == ',')
        {
            ...
            {
                seen_comma = 1;
                notify_std (&dtp->common, FTN_STD_GNU,
                    _("Comma in formatted numeric read."));
                break;
            }
        }
            n++;
    ...
    read_block_direct (st_parameter_dt *dtp, void *buf, size_t nbytes)
{
    ...
    while(1)
    {
        ...
        have_read_subrecord = sread (dtp->u.p.current_unit->s,
            (char*)buf + have_read_record, to_read_subrecord);
    ...
    write_buf (st_parameter_dt *dtp, void *buf, size_t nbytes)
{
    ...
    while (1)
    {
        ...
        to_write_subrecord = swrite (dtp->u.p.current_unit->s,
            (char*)buf + have_written, to_write_subrecord);
...
#define BUFLEN 100
    ...
    snprintf (buffer, BUFLEN,
        _("Expected %s for item %d in formatted transfer, got %s"),
    ...
    check_dtio_proc (st_parameter_dt *dtp, const fnode *f)
{
    ...
    snprintf (buffer, BUFLEN,
        _("Missing DTIO procedure or intrinsic type passed for item %d in formatted transfer"),
...
#define BUFLEN 100
    ...
    /* Adjust item_count before emitting error message.  */
    snprintf (buffer, BUFLEN,
        _("Expected numeric type for item %d in formatted transfer, got %s"),
    ...
    for (;;)
    {
        ...
        if (f == NULL)
        {
            ...
            if (unlikely (n > 0))
                ...
                _("Insufficient data descriptors in format after reversion"));
            return;
        }
        ...
        case FMT_B:
            if (n == 0)
                goto need_read_data;
            if (!(compile_options.allow_std & FTN_STD_GNU)
                && require_numeric_type (dtp, type, f))
                return;
            if (!(compile_options.allow_std & FTN_STD_F2008)
        ...
        case FMT_STRING:
            consume_data_flag = 0;
            format_error (dtp, f, _("Constant string in input format"));
            return;
    ...
    data_transfer_init (st_parameter_dt *dtp, int read_flag)
{
    ...
    if (dtp->u.p.current_unit == NULL)
    {
        ...
        generate_error (&dtp->common, LIBERROR_BAD_OPTION,
            _("Unit number is negative and unit was not already opened with OPEN(NEWUNIT=...)"));
        return;
    }
    ...
    if (read_flag && dtp->u.p.current_unit->flags.action == ACTION_WRITE)
    {
        generate_error (&dtp->common, LIBERROR_BAD_ACTION,
            _("Cannot read from file opened for WRITE"));
        return;
    }
    ...
    {
        generate_error (&dtp->common, LIBERROR_BAD_ACTION,
            _("Cannot write to file opened for READ"));
        return;
    }
        ...
        != 0)
    {
        generate_error (&dtp->common, LIBERROR_OPTION_CONFLICT,
            _("Format present for UNFORMATTED data transfer"));
        return;
    }
    ...
    if ((cf & IOPARM_DT_HAS_NAMELIST_NAME) != 0 && dtp->u.p.ionml != NULL)
    {
        ...
        {
            generate_error (&dtp->common, LIBERROR_OPTION_CONFLICT,
                _("A format cannot be specified with a namelist"));
            return;
        }
    }
    ...
    {
        generate_error (&dtp->common, LIBERROR_OPTION_CONFLICT,
            _("Missing format for FORMATTED data transfer"));
        return;
    }
    ...
    {
        generate_error (&dtp->common, LIBERROR_OPTION_CONFLICT,
            _("Internal file cannot be accessed by UNFORMATTED data transfer"));
        return;
    }
        ...
        && (cf & IOPARM_DT_HAS_REC) == 0)
    {
        generate_error (&dtp->common, LIBERROR_MISSING_OPTION,
            _("Direct access data transfer requires record number"));
        return;
    }
    ...
    if (dtp->u.p.current_unit->flags.access == ACCESS_SEQUENTIAL)
    {
        ...
        {
            generate_error (&dtp->common, LIBERROR_OPTION_CONFLICT,
                _("Record number not allowed for sequential access data transfer"));
            return;
        }
        ...
        {
            generate_error (&dtp->common, LIBERROR_OPTION_CONFLICT,
                _("Sequential READ or WRITE not allowed after EOF marker, possibly use REWIND or BACKSPACE"));
            return;
        }
    }
    ...
    dtp->u.p.advance_status
        ...
        _("Bad ADVANCE parameter in data transfer statement"));
    ...
    if (dtp->u.p.advance_status != ADVANCE_UNSPECIFIED)
    {
        ...
        {
            generate_error (&dtp->common, LIBERROR_OPTION_CONFLICT,
                _("ADVANCE specification conflicts with sequential access"));
            return;
        }
        ...
        {
            generate_error (&dtp->common, LIBERROR_OPTION_CONFLICT,
                _("ADVANCE specification conflicts with internal file"));
            return;
        }
        ...
        {
            generate_error (&dtp->common, LIBERROR_OPTION_CONFLICT,
                _("ADVANCE specification requires an explicit format"));
            return;
        }
    }
    ...
    if (read_flag)
    {
        ...
        {
            generate_error (&dtp->common, LIBERROR_MISSING_OPTION,
                _("EOR specification requires an ADVANCE specification of NO"));
            return;
        }
        ...
        {
            generate_error (&dtp->common, LIBERROR_MISSING_OPTION,
                _("SIZE specification requires an ADVANCE specification of NO"));
            return;
        }
    }
    else
    {				/* Write constraints.  */
        ...
        {
            generate_error (&dtp->common, LIBERROR_OPTION_CONFLICT,
                _("END specification cannot appear in a write statement"));
            return;
        }
        ...
        {
            generate_error (&dtp->common, LIBERROR_OPTION_CONFLICT,
                _("EOR specification cannot appear in a write statement"));
            return;
        }
        ...
        {
            generate_error (&dtp->common, LIBERROR_OPTION_CONFLICT,
                _("SIZE specification cannot appear in a write statement"));
            return;
        }
    }
    ...
    dtp->u.p.current_unit->decimal_status
        ...
        decimal_opt, _("Bad DECIMAL parameter in data transfer statement"));
    ...
    dtp->u.p.current_unit->round_status
        ...
        round_opt, _("Bad ROUND parameter in data transfer statement"));
    ...
    dtp->u.p.sign_status
        ...
        _("Bad SIGN parameter in data transfer statement"));
    ...
    dtp->u.p.blank_status
        ...
        blank_opt,
        _("Bad BLANK parameter in data transfer statement"));
    ...
    dtp->u.p.current_unit->delim_status
        ...
        delim_opt, _("Bad DELIM parameter in data transfer statement"));
    ...
    dtp->u.p.current_unit->pad_status
        ...
        _("Bad PAD parameter in data transfer statement"));
    ...
    if (((cf & IOPARM_DT_HAS_POS) != 0))
    {
        if (is_stream_io (dtp))
        {
            ...
            {
                generate_error (&dtp->common, LIBERROR_BAD_OPTION,
                    _("POS=specifier must be positive"));
                return;
            }
        ...
        {
            generate_error (&dtp->common, LIBERROR_BAD_OPTION,
                _("POS=specifier not allowed, Try OPEN with ACCESS='stream'"));
            return;
        }
    }
    ...
    if ((cf & IOPARM_DT_HAS_REC) != 0)
    {
        ...
        {
            generate_error (&dtp->common, LIBERROR_BAD_OPTION,
                _("Record number must be positive"));
            return;
        }
        ...
        {
            generate_error (&dtp->common, LIBERROR_OPTION_CONFLICT,
                _("Record number not allowed for stream access data transfer"));
            return;
        }
    }
    ...
    if (read_flag)
    {
        ...
        {
            generate_error (&dtp->common, LIBERROR_BAD_OPTION,
                _("Cannot READ after a nonadvancing WRITE"));
            return;
        }
    }
    else
